...
<div id="pozadina">
	...
	<div id="sredina">
		...
		if(isset($_REQUEST['id']))
		{
			...
			echo "<div id='objedinjeno'><p style='padding-top:20px;font-size:35px;' align='center'>".$r1['izvodjac']." - ".$r1['naziv_pesme']."</p></br></br></br>";
			echo "<div id='pesma_tekst'>".$r1['tekst_pesme']."<br/><br/><p style='color:#3434c3;' align='center'>Hvala ".$r1['korisnicko_ime']."</p></div>";
			
			if($r1['link']!=null)
			{	
				if(substr(urldecode($r1['link']),0,17)=='https://youtu.be/')
				echo "<div id='link_pesme'><iframe width='420' height='315' src='https://www.youtube.com/embed/".substr(urldecode($r1['link']),17)."' allowfullscreen></iframe></div></div>";
				else
		...
		{
		?>
		<h1 style='padding-top:10px;text-align:center;'>Najbolji tekstovi folk pesama</h1>
		<?php 
			...
			$limit=5;
				...
				$rez1=mysqli_query($konekcija, "SELECT * FROM tekstovi t JOIN zanrovi z ON t.id_zanra=z.id_zanra WHERE z.naziv_zanra='Folk' AND verifikacija=1")  or die(mysqli_error($konekcija));
			...
			{
				$upit = "SELECT * FROM tekstovi t JOIN zanrovi z ON t.id_zanra=z.id_zanra WHERE z.naziv_zanra='Folk' AND verifikacija=1 LIMIT ".$start.",".$limit;
				$rez = mysqli_query($konekcija,$upit);
				echo "<div id='pesme'>";
				while($r=mysqli_fetch_array($rez))
				{
				...
				echo "</div>";
				pagination_songs($strana,$broj_strana,'folk');
			}
			}
		?>
		</div>	
