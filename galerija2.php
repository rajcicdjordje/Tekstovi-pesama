...
<div id="pozadina">
	...
	<div id="sredina">
		...
		<?php
			...
			if(isset($_REQUEST['btnPrikazi']) || isset($_REQUEST['p']))
			{
				...
				$rez1=mysqli_query($konekcija, "SELECT * FROM slike WHERE id_galerije=".$idGalerije)  or die(mysqli_error($konekcija));
				$red1=mysqli_num_rows($rez1);
				$broj_strana=ceil($red1/$limit);
				
				
				$upit_ispis = "SELECT * FROM slike WHERE id_galerije=".$idGalerije." LIMIT ".$start.",".$limit;
				...
				else
				{
					...
					pagination($strana,$broj_strana,$idGalerije);
				}
			}
		?>
		</div>
