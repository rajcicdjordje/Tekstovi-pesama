...
<div id="pozadina">
	...
	<div id="sredina">
		...
		if(isset($_REQUEST['id']))
		{
			...
			echo "<div id='objedinjeno'><div id='pesma_tekst'>".$r1['tekst_pesme']."<br/><br/><p style='color:#3434c3;' align='center'>Hvala ".$r1['korisnicko_ime']."</p></div>";
		...
		{
		?>
		<h1 style='padding-top:10px;text-align:center;'>Najbolji tekstovi pop pesama</h1>
		<?php 
			...
			$limit=5;
				...
				$rez1=mysqli_query($konekcija, "SELECT * FROM tekstovi t JOIN zanrovi z ON t.id_zanra=z.id_zanra WHERE z.naziv_zanra='Pop' AND verifikacija=1")  or die(mysqli_error($konekcija));
			...
			{
				$upit = "SELECT * FROM tekstovi t JOIN zanrovi z ON t.id_zanra=z.id_zanra WHERE z.naziv_zanra='Pop' AND verifikacija=1 LIMIT ".$start.",".$limit;
				$rez = mysqli_query($konekcija,$upit);
				echo "<div id='pesme'>";
				while($r=mysqli_fetch_array($rez))
				{
				...
				echo "</div>";
				pagination_songs($strana,$broj_strana,'pop');
			}
			}
		?>
		</div>	
